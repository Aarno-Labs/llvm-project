...
//
// This file contains the mechanically extracted owner-state delta, graph,
// suffix-observer, and transition-gateway logic.  RefoldEngine still owns the
// orchestration state and edit emission; this module only evaluates and records
// theorem-facing owner-state proof facts.
//
//===----------------------------------------------------------------------===//
...
bool RefoldEngine::OwnerStateDeltaHasUnmodeledState(
...
bool RefoldEngine::OwnerStateDeltaHasUnknownPragmaState(
    const OwnerStateDelta &delta) {
...
bool RefoldEngine::OwnerStateDeltaMutatesAnyState(
    const OwnerStateDelta &delta) {
  return delta.Mutates.MutatesAnyState() || delta.Exit.MutatesAnyState();
}

RefoldEngine::OwnerObserverSummary
...
RefoldEngine::DirectStateCheckKind
RefoldEngine::DirectStateCheckKindForComponent(OwnerStateComponent component) {
  switch (component) {
...
RefoldEngine::DirectStateCheckKind
RefoldEngine::DirectStateCheckKindForGraphNode(OwnerStateGraphNodeKind kind) {
  switch (kind) {
...
RefoldEngine::DirectStateComponentForGraphNode(OwnerStateGraphNodeKind kind) {
  switch (kind) {
  case OwnerStateGraphNodeKind::MacroDefineEvent:
  case OwnerStateGraphNodeKind::MacroUndefEvent:
  case OwnerStateGraphNodeKind::MacroInvocation:
...
void RefoldEngine::AuditDirectStateCheckClosure(
...
RefoldEngine::OwnerStateDelta
RefoldEngine::BuildOwnerStateDelta(const Owner &owner) const {
  OwnerStateFacts facts;
  const bool auditDirectState = IsNoLegacyAuditEnabled();
  ...
  auto recordCounterInvocationEvents =
      [&](const RefoldModel::MacroInvocation &macro) {
        ...
        for (const auto &range : CounterOutputRangesForInvocation(macro)) {
          facts.AddCounterEvent(BuildCounterEventIdentity(
...
RefoldEngine::StateComponentsMutatedByDelta(
...
RefoldEngine::StateTransitionProof
RefoldEngine::CheckStateTransitionAcrossEditBoundary(
  ...
  auto requestTerminal = [&](TerminalFallbackProofFailure failure,
    ...
    if (!AuditTerminalFallbackProofFailure(failure, request.stage))
    ...
    RequestTerminalFallback(std::move(failure), request.stage, message);
...
RefoldEngine::SuffixStabilityWitness
RefoldEngine::BuildStateTransitionWitness(SuffixStabilityWitnessKind kind,
                                          OwnerStateComponent component,
...
std::string
RefoldEngine::FormatCounterEventForWitness(const CounterEventIdentity &event) {
...
bool RefoldEngine::IsLineControlStateComponent(
    OwnerStateComponent component) {
  switch (component) {
  case OwnerStateComponent::LineNumber:
  case OwnerStateComponent::FileState:
...
std::string RefoldEngine::FormatLineControlEventForWitness(
...
std::string RefoldEngine::FormatBuiltinLocationObservationForWitness(
