...
//
// TU-anchor and TU byte-span proofs.
//
// This file proves TU insertion anchors and TU byte spans, and certifies each
// local TUAnchorWitness into the normalized AcceptedResultCandidate shape used
// by the existing selector/audit machinery.  It is not a general proof
// lattice: it does not compare candidates, assemble text edits, or decide
// which owner an A/B hunk belongs to.
//
//===----------------------------------------------------------------------===//
...
RefoldTUAnchorProof::RefoldTUAnchorProof(const RefoldTheoremAudit &theoremAudit,
    ...
    : theoremAudit_(theoremAudit),
      proofSummaryBuilder_(
          RefoldProofSummaryBuilder::Dependencies{deps.bTokens}),
      deps_(std::move(deps)) {
...
bool RefoldTUAnchorProof::CollectDirectTUMacroRepairEvidence(
...
bool RefoldTUAnchorProof::CollectDirectTUMacroRepairEvidenceOrEmpty(
    uint64_t begin, uint64_t end,
    std::vector<const PreprocessingStructureInterval *> &intervals) const {
  intervals.clear();
...
bool RefoldTUAnchorProof::DirectTUEnvelopeRetainsMacroRepairEvidence(
...
bool RefoldTUAnchorProof::ProveDirectTUGapWithMacroRepairEvidence(
...
bool RefoldTUAnchorProof::ValidateDirectTUEnvelope(
    StringRef tuPath, uint64_t begin, uint64_t end) const {
  const RefoldPreprocessingStructureIndex &structure =
      deps_.preprocessingStructureIndex;
  if (end < begin || end > deps_.tuSourceBytes.size() ||
...
bool RefoldTUAnchorProof::IsPPGapAtSelectedConditionalArmExit(
...
std::optional<uint64_t> RefoldTUAnchorProof::AnchorToExactSlotBoundaryFromPPGap(
  ...
  auto recordAcceptedAnchorCandidate =
      [&](AcceptedPathKind path, const TUAnchorWitness &anchorWitness) {
        AcceptedResultCandidate candidate =
            BuildAcceptedTUAnchorCandidate(path, anchorWitness);
...
RefoldTUAnchorProof::FindExactSlotBoundaryFromPPGap(StringRef tuPath,
                                                    uint64_t ppGap) const {
...
std::optional<uint64_t>
RefoldTUAnchorProof::IncludeIdCoveringPPIndex(uint64_t pp) const {
...
RefoldTUAnchorProof::FindProvableTUInsertionAnchor(
...
RefoldTUAnchorProof::ProjectTUByteEnvelopeForOwnership(
...
std::optional<TUByteSpanPlan>
RefoldTUAnchorProof::PlanTUByteSpan(uint64_t a0, uint64_t a1,
...
bool RefoldTUAnchorProof::InsertionSuppliesMacroBoundaryLiteral(
...
bool RefoldTUAnchorProof::ValidateTUOwnerRealizationCarrier(
