...
// Shared macro-state proof primitives.  These helpers are used by TU
// macro-state carry, materialized-header stabilization, include repair, and
// fallback proof code.  Keeping their definitions in this module prevents the
// policy split that previously made macro-state observation hard to audit.
...
std::optional<RefoldEngine::StabilizedMaterializedHeaderMacroPatch>
RefoldEngine::StabilizeMaterializedHeaderMacroPatchReplay(
    const MacroPatch &mp, uint64_t mpEnd, StringRef headerPath,
    uint64_t includeId, StringRef bytes, ArrayRef<TextEdit> stagedEdits) const {
  ...
  auto materializedHeaderRangeOverlapsStagedEdit = [&](uint64_t begin,
                                                       uint64_t end) {
    for (const TextEdit &edit : stagedEdits)
