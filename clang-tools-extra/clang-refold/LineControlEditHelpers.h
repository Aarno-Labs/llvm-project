...
//
// Private helper routines for final line-control candidate bookkeeping shared by
// the line-observer, include-materialization, and text-edit assembly translation
// units.  These helpers intentionally preserve the old TailUtilities behavior:
// they build, shift, and deduplicate proof-carrying candidate records without
// owning any RefoldEngine state.
//
//===----------------------------------------------------------------------===//
...
inline bool isPhysicalLineControlDirectiveLine(StringRef line) {
  line = line.trim();
  if (!line.starts_with("#"))
    return false;
...
findLastLineControlDirectiveRangeBefore(StringRef out,
