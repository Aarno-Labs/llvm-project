...
public:
  ...
  // directives bind.
  //
  // This is a precedence rule, not a soundness condition.  Splitting at a
  // preserved macro-state gap leaves the directive exactly where it was, so
  // source outside the hunk sees an unchanged directive position either way;
  // the placement obligations proved at the caller are what make the split
  // sound.  What this decides is *who owns the seam*.  The macro-state liveness
  // planner is the only stage that models resurrection, undef/restore, and
  // replay ordering for a consumed directive, and it engages exactly when the
  // name still matters downstream.  Reporting true there leaves the seam to it,
  // so a hunk it already handles keeps the placement it proves rather than
  // having the tiler decide by an alignment that models none of that.
  //
  ...
  BuildPhysicalSourceRunPlan(const diffutils::Hunk &h) const {
    ...
    for (uint64_t pp = h.aStart; pp < h.aEnd; ++pp) {
      ...
      if (previousEnd < spellingBegin) {
        ...
        for (const PreprocessingStructureInterval *interval :
          ...
          if (!interval || interval->ownerIncludeId != tokenOwner->includeId) {
            return std::nullopt;
          }
        ...
        crossesProtectedStructure = !gapProof->protectedIntervals.empty();
      }
    ...
    if (plan.runs.empty() ||
        ...
        plan.protectedGapFacts.size() != plan.protectedGaps.size()) {
      return std::nullopt;
    }
  ...
  BuildStructuralTilingWitness(const diffutils::Hunk &h,
    ...
    witness.sharedEmptyBEnvelopeProven = partition.sharedEmptyBEnvelopeProven;
    witness.sharedEmptyBBoundary = partition.sharedEmptyBBoundary;
    witness.preservedStateChainComposed = partition.preservedStateChainComposed;
    ...
    auto formatOwnerSignature = [&](const Owner &owner) {
      return llvm::formatv("kind={0}:include={1}:macro={2}:macro_directive={3}:"
    ...
    auto formatSourceSignature = [&](const OwnerSourceRange &source) {
      return llvm::formatv("path={0}:range=[{1},{2}):include={3}", source.path,
    ...
    compositionOS
        << "tiling=" << witnessId << ":reason=" << toString(partition.reason)
        << ":A=[" << h.aStart << ',' << h.aEnd << "):B=[" << h.bStart << ','
        << h.bEnd << ")"
        << ":unique_partition=" << (partition.uniquePartition ? 1 : 0)
        << ":state_transitions=" << (witness.stateTransitionsComposed ? 1 : 0)
        << ":disjoint_from_edits="
        << (witness.preservedGapsDisjointFromEdits ? 1 : 0)
        << ":source_cover=" << (partition.sourceByteCoverComplete ? 1 : 0)
        << ":physical_runs=" << partition.physicalSourceRunCount
        << ":physical_runs_proven="
        << (partition.physicalSourceRunsProven ? 1 : 0) << ":minimum_fragments="
        << (partition.uniqueMinimumFragmentPartition ? 1 : 0)
        << ":unique_boundary_projection="
        << (partition.uniqueBoundaryProjectionProven ? 1 : 0)
        << ":boundary_projection_count=" << partition.boundaryProjections.size()
        << ":shared_empty_b=" << (partition.sharedEmptyBEnvelopeProven ? 1 : 0)
        << ":shared_empty_b_boundary=" << partition.sharedEmptyBBoundary
        ...
        << (partition.preservedGapsDisjointFromTokenSegments ? 1 : 0);
    for (const StructuralBoundaryProjectionWitness &projection :
    ...
    for (const PartitionEdge &edge : path) {
      ...
      segmentWitness.producerConditionalArmId = edge.producerConditionalArmId;
      segmentWitness.macroStatePlacementInsensitiveProven =
          edge.macroStatePlacementInsensitiveProven;
      segmentWitness.allowEmptyBEnvelope = edge.allowEmptyBEnvelope;
      segmentWitness.sourceBytesPreservedUnchanged =
          edge.IsStateGap() &&
          edge.gapDisposition == StructuralGapDisposition::PreservedInPlace;
      segmentWitness.canonicalStateTransition = buildOwnerTransitionProof(edge);
      ...
      compositionOS
          << ";seg" << segmentIndex << "{kind=" << toString(edgeKind) << ":A=["
          << edge.aStart << ',' << edge.aEnd << "):B=[" << edge.bStart << ','
          << edge.bEnd << "):empty_b=" << (edge.allowEmptyBEnvelope ? 1 : 0)
          << ":gap_disposition=" << toString(edge.gapDisposition)
          << ":protected_gap=" << (edge.protectedPreprocessingStructure ? 1 : 0)
          << ":structure_identity="
          << (segmentWitness.protectedStructureIdentityRecorded ? 1 : 0)
          << ":structure_kind="
          << toString(segmentWitness.protectedStructureKind)
          << ":producer_kind=" << toString(segmentWitness.producerIdentityKind)
          << ":producer_item="
          << formatOptionalId(segmentWitness.producerItemId)
          << ":producer_cond_group="
          << formatOptionalId(segmentWitness.producerConditionalGroupId)
          << ":producer_cond_arm="
          << formatOptionalId(segmentWitness.producerConditionalArmId)
          << ":bytes_unchanged="
          << (segmentWitness.sourceBytesPreservedUnchanged ? 1 : 0)
          << ":protected_outside="
          << (segmentWitness.protectedStructurePreservedOutsideSegment ? 1 : 0)
  ...
  ProveClosedStateGapTransition(const PartitionEdge &prev,
    ...
    if (protectedBinding.value_or(false) &&
      ...
      for (PartitionEdge &gap : transition.gaps) {
        gap.gapDisposition = StructuralGapDisposition::PreservedInPlace;
      }
...
/// Borrow the producer facts the final printed-pragma placement reads.
PrintedPragmaHunkPlacementDependencies printedPragmaPlacementDeps(
    const RefoldStructuralHunkTilingPlanner::Dependencies &deps) {
  return {deps.model, deps.macroTopology, deps.sidebandPragmaLinePairings,
          deps.sidebandPragmaEdits, deps.printedPragmaCarriers};
}
...
RefoldStructuralHunkTilingPlanner::Plan(std::vector<diffutils::Hunk> hunks) {
  ...
  if (hunks.empty()) {
    std::optional<PrintedPragmaPlacementViolation> violation =
        enforcePrintedPragmaPlacement(printedPragmaPlacementDeps(deps_), hunks,
                                      {});
