//===--- SourceEnvelopeProof.h ---------------------*- C++ -*-===//
//
// Shared source-envelope proof helpers for RefoldEngine edit construction.
//
// These utilities prove that caller-supplied source pieces form a deterministic,
// non-overlapping envelope and that every intervening byte gap is discharged by
// a caller-specific neutral-range proof.  They are intentionally engine-private:
// the helpers support include materialization and expansion fallback edit
// tiling, but they are not owner-state transition logic.
//
//===----------------------------------------------------------------------===//

#ifndef LLVM_CLANG_TOOLS_EXTRA_CLANG_REFOLD_SOURCEENVELOPEPROOF_H
#define LLVM_CLANG_TOOLS_EXTRA_CLANG_REFOLD_SOURCEENVELOPEPROOF_H
...
/// Byte envelope covered by normalized source-envelope proof pieces.
struct SourceEnvelopeProof {
  uint64_t begin = 0;
  uint64_t end = 0;
};

/// Source-envelope interval accessors. Structural pieces expose begin/end;
/// merged TU byte ranges may be represented as plain pairs.
...
/// Normalize and tile one source gap with caller-specific neutral-gap and piece
/// consumption proofs.
template <typename PieceT, typename KindLess, typename NestedCovered,
...
#endif // LLVM_CLANG_TOOLS_EXTRA_CLANG_REFOLD_SOURCEENVELOPEPROOF_H
