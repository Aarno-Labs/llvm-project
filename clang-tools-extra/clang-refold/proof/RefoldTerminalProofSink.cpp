...
//
// This TU owns the mutable per-refold request sink plus the terminal
// fallback proof vocabulary (toString/normalize/factory functions and the
// TerminalFallbackFailureContext / TerminalFallbackProofFailure /
// TerminalFallbackWitness / TerminalFallbackRequest method definitions).
// The theorem audit ledger lives next door in RefoldTheoremAudit.{h,cpp}.
//
//===----------------------------------------------------------------------===//
...
std::string TerminalFallbackFailureContext::ToString() const {
  ...
  if (sourceBegin || sourceEnd)
    add("sourceBytes",
        formatv("[{0},{1})", sourceBegin.value_or(0), sourceEnd.value_or(0)));
  if (aTokenBegin || aTokenEnd)
    add("aTokens",
        formatv("[{0},{1})", aTokenBegin.value_or(0), aTokenEnd.value_or(0)));
  if (bTokenBegin || bTokenEnd)
    add("bTokens",
        formatv("[{0},{1})", bTokenBegin.value_or(0), bTokenEnd.value_or(0)));
  os.flush();
  return text;
}
...
const TerminalFallbackProofFailure *
TerminalFallbackWitness::PrimaryFailure() const {
...
std::string TerminalFallbackRequest::ToString() const {
  return formatv("terminal fallback requested: action=raw-b-emission "
...
//===----------------------------------------------------------------------===//
// Terminal fallback request sink.
//===----------------------------------------------------------------------===//
