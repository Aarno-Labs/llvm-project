...
RefoldProofLattice::RefoldProofLattice(
    ...
    Hooks hooks)
    : hooks_(std::move(hooks)), theoremAudit_(theoremAudit),
      tokenText_(tokenText),
      argTextRecovery_(argTextRecovery), macroTopology_(macroTopology),
      ownerStateProof_(ownerStateProof),
      model_(model), bSource_(bSource), bToks_(bToks),
      sourceMapper_(sourceMapper), lastTheoremAudit_(lastTheoremAudit),
      strict_(strict), proofAuditMode_(proofAuditMode),
      terminalSink_(terminalSink), tuEdits_(tuEdits),
      mixedOwnerTilingSegmentBindings_(mixedOwnerTilingSegmentBindings),
      mixedOwnerTilingWitnesses_(mixedOwnerTilingWitnesses) {}

::clang::refold::AcceptancePathInventory
RefoldProofLattice::InventoryMacroPatchProofAcceptancePath(
...
::clang::refold::AcceptancePathInventory
RefoldProofLattice::InventoryMacroPatchAcceptancePath(const MacroPatch &patch) const {
...
::clang::refold::AcceptancePathInventory
RefoldProofLattice::BuildAcceptancePathInventory(AcceptedPathKind currentPath) const {
...
RefoldProofLattice::BuildTheoremProofClassForAcceptedPath(
...
void RefoldProofLattice::RequireAcceptedPathBaseline(
...
RefoldProofLattice::BuildAcceptedPathBaselineDischarge(
...
void RefoldProofLattice::ConfigureProofSummary(
...
bool RefoldProofLattice::ProofSummaryRequiresOwnerRealizationWitness(
...
void RefoldProofLattice::FinalizeProofSummary(ProofSummary &summary) const {
...
void RefoldProofLattice::RequireIncludeZeroWidthAnchor(
...
RefoldProofLattice::ClassifyMacroPatchProof(const MacroPatch &patch) const {
...
void RefoldProofLattice::RefreshMacroPatchDerivedProofWitnesses(
...
void RefoldProofLattice::SyncMacroPatchProofSummary(MacroPatch &patch) const {
  RefreshMacroPatchDerivedProofWitnesses(patch);
  patch.proofSummary = ClassifyMacroPatchProof(patch);
}

::clang::refold::MacroPatchProof RefoldProofLattice::MakeMacroPatchProof(
...
void RefoldProofLattice::StampSelectedMacroPatchCandidate(
  ...
  if (candidate.kind != AcceptedResultCandidateKind::MacroPatch) {
    theoremAudit_.ReportNoLegacyAuditFinding(
...
RefoldProofLattice::WitnessFamilyForAcceptedPath(AcceptedPathKind path) {
  switch (path) {
  case AcceptedPathKind::MacroArgsOnlyStandard:
  case AcceptedPathKind::MacroArgsOnlyPairedPureInsertion:
...
bool RefoldProofLattice::IsResolverAuthoritativeWitnessFamily(
...
bool RefoldProofLattice::IsResolverAuthoritativeWitness(
...
RefoldProofLattice::WitnessProducerKindForAcceptedPath(AcceptedPathKind path) {
  switch (path) {
  case AcceptedPathKind::MacroArgsOnlyStandard:
  case AcceptedPathKind::MacroArgsOnlyPairedPureInsertion:
...
::clang::refold::WitnessBoundaryClass
RefoldProofLattice::WitnessBoundaryClassForAcceptedCandidate(
...
std::string RefoldProofLattice::FormatWitnessTraceHash(StringRef text) {
...
void RefoldProofLattice::AttachLineControlObserverWitness(
  ...
  auto recordLineControlEvent = [&](const LineControlStateIdentity &event,
    ...
    appendSig(witness.stateSignature,
              llvm::formatv("{0}:{1}", bucket,
                            ownerStateProof_.FormatLineControlEventForWitness(event))
                  .str());
  };
  ...
  auto recordBuiltinObservation = [&](const BuiltinLocationObservation &obs,
    ...
    appendSig(witness.observerSignature,
              llvm::formatv("{0}:{1}", bucket,
                            ownerStateProof_.FormatBuiltinLocationObservationForWitness(obs))
                  .str());
  };
    ...
    for (const MixedOwnerTilingSegmentWitness &segment : tiling.segments) {
      ...
      for (const SuffixStabilityWitness &suffix :
        ...
        const OwnerStateComponent component =
            ownerStateProof_.ComponentNamedBySuffixStabilityWitness(suffix);
        if (!ownerStateProof_.IsLineControlStateComponent(component))
          continue;
...
void RefoldProofLattice::AttachCounterStateWitness(
    AcceptedResultCandidate &candidate) const {
  ...
  auto recordCounterEvent = [&](const CounterEventIdentity &event,
    ...
    appendSig(witness.consumptionSignature,
              llvm::formatv("{0}:{1}", bucket,
                            ownerStateProof_.FormatCounterEventForWitness(event))
                  .str());
  };
  ...
  auto recordSuffixWitness = [&](const SuffixStabilityWitness &suffix,
                                 llvm::StringRef bucket) {
    if (!ownerStateProof_.SuffixStabilityWitnessNamesComponent(suffix,
...
::clang::refold::WitnessEquivalenceKey RefoldProofLattice::BuildWitnessEquivalenceKey(
  ...
  auto knownRangeHash = [&](llvm::StringRef label, uint64_t begin,
    ...
    return WitnessEquivalenceDimension::Known(
        llvm::formatv("{0}:[{1},{2}):{3}", label, begin, end,
                      FormatWitnessTraceHash(sourceMapper_.SliceBSource(begin, end)))
            .str());
  };
  ...
  auto macroRepairReplayStateNeutralityValue = [&]() {
    ...
    return llvm::formatv(
               ...
               FormatWitnessTraceHash(sourceMapper_.SliceBSource(candidate.targetBTokStart,
  ...
  auto ownerStateDeltaSignature = [&](const OwnerRealizationWitness &owner) {
    ...
    for (const SuffixStabilityWitness &suffix : owner.stateWitnesses)
      os << "state_witness{" << toString(suffix.kind) << ':'
         << toString(ownerStateProof_.ComponentNamedBySuffixStabilityWitness(suffix)) << "};";
    os.flush();
  ...
  auto ownerRealizationHasKnownIncludeState =
      [&](const OwnerRealizationWitness &owner) {
        ...
        if (deltaHasAnyMissingFacts(owner.closure.stateIn) ||
            ...
            ownerStateProof_.OwnerStateDeltaHasUnknownPragmaState(owner.closure.stateIn) ||
            ownerStateProof_.OwnerStateDeltaHasUnknownPragmaState(owner.closure.stateOut) ||
  ...
  auto ownerRealizationHasKnownBuiltinLocationObservationState =
      [&](const OwnerRealizationWitness &owner) {
        ...
        const OwnerStateDelta &in = owner.closure.stateIn;
        const OwnerStateDelta &out = owner.closure.stateOut;
        if (ownerStateProof_.OwnerStateDeltaMutatesAnyState(in) ||
            ownerStateProof_.OwnerStateDeltaMutatesAnyState(out))
          return false;
...
::clang::refold::WitnessCanonicalCost RefoldProofLattice::BuildWitnessCanonicalCost(
    const AcceptedResultCandidate &candidate) const {
...
RefoldProofLattice::BuildRefoldWitness(const AcceptedResultCandidate &candidate,
                                 llvm::StringRef role,
...
::clang::refold::WitnessResolverMode RefoldProofLattice::GetWitnessResolverMode() const {
  if (strict_)
...
bool RefoldProofLattice::ShouldEmitProofLog() const {
...
RefoldProofLattice::ClassifyTerminalFallbackFailure(
...
::clang::refold::WitnessFallbackClass RefoldProofLattice::ClassifyIncompleteWitnessKeys(
...
void RefoldProofLattice::PopulateWitnessClosureLedger(
...
::clang::refold::WitnessFallbackClass RefoldProofLattice::ClassifyResolverFallbackReason(
...
RefoldProofLattice::StrictDomainObligationForFallbackClass(
...
RefoldProofLattice::ClassifyStrictDomainForResolver(
...
::clang::refold::WitnessStrictDomainDecision
RefoldProofLattice::ClassifyStrictDomainForTerminalFallback(
    const TerminalFallbackProofFailure &failure) {
  using DomainClass = WitnessStrictDomainClass;
  using FailureReason = TerminalFallbackFailureReason;
  using FallbackClass = WitnessFallbackClass;
...
void RefoldProofLattice::TraceWitnessStrictDomain(
...
void RefoldProofLattice::TraceWitnessClosureLedger(
...
void RefoldProofLattice::TraceWitnessResolverDecision(
    const WitnessResolverDecision &decision) const {
...
void RefoldProofLattice::TraceWitnessCompositionDecision(
...
RefoldProofLattice::ResolveWitnessComposition(
    llvm::StringRef role,
    ArrayRef<std::pair<size_t, RefoldWitness>> selectableWitnesses,
...
RefoldProofLattice::ResolveWitnessesForSelection(
  ...
  theoremAudit_.RecordWitnessResolverTheoremAudit(decision);

  TraceWitnessResolverDecision(decision);
  return decision;
}

void RefoldProofLattice::TraceWitnessEmitted(const RefoldWitness &witness) const {
  if (!ShouldEmitProofLog())
    return;
...
void RefoldProofLattice::TraceWitnessRejected(const RefoldWitness &witness,
...
void RefoldProofLattice::TraceWitnessAmbiguity(llvm::StringRef role,
...
void RefoldProofLattice::TraceWitnessSelectionProbe(
...
void RefoldProofLattice::TraceWitnessChosen(const RefoldWitness &witness,
...
void RefoldProofLattice::TraceWitnessFallback(
...
void RefoldProofLattice::AttachMixedOwnerTilingWitnessForTokenEnvelope(
...
RefoldProofLattice::TryBuildOwnerRealization(OwnerRealizationEvidenceKind evidence,
                                       OwnerClosure closure,
                                       StringRef detail) const {
  OwnerRealizationResult result;
  result.witness.evidence = evidence;
  result.witness.closure = ownerStateProof_.AttachCanonicalStateSummary(std::move(closure));
  ...
  for (OwnerStateComponent component :
       ownerStateProof_.StateComponentsMutatedByDelta(result.witness.closure.stateIn))
    addUniqueComponent(mutatedComponents, component);
  for (OwnerStateComponent component :
       ownerStateProof_.StateComponentsMutatedByDelta(result.witness.closure.stateOut))
    addUniqueComponent(mutatedComponents, component);
  ...
  const bool mutatesOrCarriesUnmodeledState =
      ownerStateProof_.OwnerStateDeltaMutatesAnyState(result.witness.closure.stateIn) ||
      ownerStateProof_.OwnerStateDeltaMutatesAnyState(result.witness.closure.stateOut);
...
void RefoldProofLattice::ApplyOwnerRealizationResultToProofSummary(
...
::clang::refold::OwnerRealizationResult
RefoldProofLattice::BuildMacroWholeCoverOwnerRealization(
...
::clang::refold::OwnerRealizationResult
RefoldProofLattice::BuildIncludeOwnerRealization(
  ...
  const OwnerStateBoundary includeBoundary =
      ownerStateProof_.IncludeStateBoundaryForIncludeSite(include);
  ...
  (void)ownerStateProof_.CheckStateTransitionAcrossEditBoundary(
      ...
      ownerStateProof_.BuildStateTransitionWitness(SuffixStabilityWitnessKind::ClosureWidening,
...
::clang::refold::OwnerRealizationResult
RefoldProofLattice::BuildTUOwnerRealization(AcceptedPathKind currentPath,
...
void RefoldProofLattice::StampMacroWholeCoverRealizationPatch(
...
::clang::refold::ProofSummary RefoldProofLattice::BuildAcceptedPathProofSummary(
...
::clang::refold::ProofSummary
RefoldProofLattice::BuildIncludePatchProofSummary(bool realizedSurface,
...
RefoldProofLattice::BuildEmittedProofFromSummary(TheoremProofClass theoremClass,
...
RefoldProofLattice::BuildCanonicalEmittedProofFromSummary(
    const ProofSummary &summary) const {
...
std::optional<::clang::refold::EmittedProof> RefoldProofLattice::BuildEmittedProof(
    const AcceptedResultCandidate &candidate) const {
...
RefoldProofLattice::NormalizeAcceptedProof(
    const AcceptedResultCandidate &candidate) const {
...
RefoldProofLattice::BuildGlobalSelectionLattice(const ProofSummary &summary) const {
...
RefoldProofLattice::BuildCompletenessContract(const ProofSummary &summary) const {
...
RefoldProofLattice::BuildTheoremDomainContract(const ProofSummary &summary) const {
...
bool RefoldProofLattice::LatticePrefers(const ProofSummary &lhs,
  ...
  auto theoremTieBreakerPrefers =
    ...
    // only order two already-normalized accepted summaries after the builder has
    // proved the witness-specific preconditions for the named rule.
...
bool RefoldProofLattice::IsSelectableAcceptedResultCandidate(
    const AcceptedResultCandidate &candidate) const {
...
bool RefoldProofLattice::AcceptedResultCandidateProofPrefers(
...
bool RefoldProofLattice::AcceptedResultCandidateCanonicalPrefers(
    const AcceptedResultCandidate &lhs,
    const AcceptedResultCandidate &rhs) const {
...
bool RefoldProofLattice::AcceptedResultCandidatePrefers(
    const AcceptedResultCandidate &lhs,
    const AcceptedResultCandidate &rhs) const {
...
std::optional<size_t> RefoldProofLattice::SelectPreferredCandidateIndex(
...
RefoldProofLattice::BuildMacroSelectionCandidate(
...
bool RefoldProofLattice::IsSelectableMacroSelectionCandidate(
...
bool RefoldProofLattice::MacroSelectionCandidatePrefers(
...
RefoldProofLattice::SelectPreferredMacroSelectionCandidate(
...
RefoldProofLattice::SelectPreferredAcceptedResultCandidate(
...
std::optional<size_t> RefoldProofLattice::SelectPreferredAcceptedResultCandidateIndex(
    ArrayRef<AcceptedResultCandidate> candidates) const {
...
::clang::refold::EmissionPathKind
RefoldProofLattice::PrimaryEmissionPathForCandidateKind(
...
void RefoldProofLattice::RefreshAcceptedCandidateEmissionPathInventory(
    AcceptedResultCandidate &candidate) const {
...
RefoldProofLattice::BuildAcceptedMacroCandidate(const MacroPatch &patch) const {
  ...
  if (const RefoldModel::MacroInvocation *inv =
          macroTopology_.FindMacroInvocationById(patch.macroId)) {
    if (!inv->stringifySpans.empty() && patch.hasMaterializedBTokenRange &&
      ...
      for (const RefoldModel::PPArgSpan &span : spans) {
        ...
        producerSig += llvm::formatv(":arg={0}:[{1},{2})", span.argIdx,
                                     span.begin, span.end)
                           .str();
        StringRef literal = sourceMapper_.SliceASource(span.begin, span.end).trim();
        std::optional<std::string> unstringified =
            argTextRecovery_.UnstringifyLiteralToArgText(
  ...
  auto macroInvocationIsInSubtreeOf = [&](
    ...
    while (true) {
      ...
      const RefoldModel::MacroInvocation *current =
          macroTopology_.FindMacroInvocationById(currentId);
  ...
  auto macroSubtreeContainsCounterInvocation = [&](uint64_t rootId) {
    if (!macroTopology_.FindMacroInvocationById(rootId))
      return true;
  ...
  if (!candidate.hasCounterStateWitness && counterNeutralRootId != 0 &&
      ...
      patch.proof.preservesInvocationStructure &&
      !tokenText_.RawIdentifierAppearsInText("__COUNTER__", patch.replacement) &&
  ...
  theoremAudit_.AuditMacroPatchProofForLegacyAuthority(patch, "BuildAcceptedMacroCandidate");
  TraceWitnessEmitted(
...
::clang::refold::AcceptedResultCandidate
RefoldProofLattice::RestampAcceptedMacroCandidateForEmission(
...
::clang::refold::AcceptedResultCandidate
RefoldProofLattice::BuildAcceptedMacroEmissionCandidate(
    const MacroPatch &patch) const {
  return RestampAcceptedMacroCandidateForEmission(
      patch, BuildAcceptedMacroCandidate(patch));
}

bool RefoldProofLattice::FinalizeSelectedMacroPatchForEmission(
  ...
  const bool rejected = theoremAudit_.RejectMissingSelectedMacroPatchCarrier(
      patch, role,
      llvm::formatv(
...
::clang::refold::AcceptedResultCandidate
RefoldProofLattice::BuildAcceptedEmittedMacroCandidate(
    const MacroPatch &patch) const {
  ...
  if (!patch.selectedAcceptedCandidate) {
    theoremAudit_.RejectMissingSelectedMacroPatchCarrier(
...
::clang::refold::AcceptedResultCandidate
RefoldProofLattice::BuildAcceptedIncludeCandidate(
    AcceptedPathKind currentPath, const IncludePatch &patch,
...
::clang::refold::ProofSummary RefoldProofLattice::BuildOwnerRealizationProofSummary(
    AcceptedPathKind currentPath,
...
::clang::refold::AcceptedResultCandidate
RefoldProofLattice::BuildAcceptedIncludeRealizationCandidate(
...
::clang::refold::AcceptedResultCandidate
RefoldProofLattice::BuildAcceptedTUTextEditCandidate(AcceptedPathKind currentPath,
...
::clang::refold::AcceptedResultCandidate
RefoldProofLattice::BuildAcceptedTerminalCandidate(
...
//
// These routines validate producer paste/stringification witnesses and normalize
// macro-local proof facts before the accepted-result lattice ranks candidate
// patches.  They do not emit edits or mutate source text.
//

bool RefoldProofLattice::MacroInvocationHasWellFormedPasteWitnesses(
...
::clang::refold::ProofDischargeRecord
RefoldProofLattice::ValidateInvocationPreservingProofImpl(
  ...
  const RefoldModel::MacroInvocation *root =
      proof.proofRootMacroId ? macroTopology_.FindMacroInvocationById(proof.proofRootMacroId)
                             : nullptr;
  ...
  if (requireTopLevelRoot && root) {
    discharge.Require(macroTopology_.GetRootMacroId(root->id) == root->id,
                      ProofObligationKind::MacroProofRootIsTopLevel,
                      ProofFailureReason::NonTopLevelMacroProofRoot);
  }
...
::clang::refold::ProofDischargeRecord
RefoldProofLattice::ValidateInvocationPreservingProof(const MacroPatch &patch) const {
  return ValidateInvocationPreservingProofImpl(
      patch, /*requireTopLevelRoot=*/true);
}

::clang::refold::ProofDischargeRecord
RefoldProofLattice::ValidateEmittedInvocationPreservingProof(
    const MacroPatch &patch) const {
  return ValidateInvocationPreservingProofImpl(
...
::clang::refold::ProofDischargeRecord
RefoldProofLattice::ValidateInvocationRealizationProof(
    const MacroPatch &patch) const {
  ProofDischargeAccumulator discharge;
  const MacroPatchProof &proof = patch.proof;
  const AcceptancePathInventory inventory =
  ...
  if (proof.kind == MacroPatchProofKind::WholeCoverRealization) {
    ...
    if (proof.kind == MacroPatchProofKind::CounterLiteral) {
      ...
      const bool counterWitnessTracked =
          ...
          ownerStateProof_.SuffixStabilityWitnessNamesComponent(*proof.suffixStability,
                                               OwnerStateComponent::Counter) &&
...
::clang::refold::ProofDischargeRecord RefoldProofLattice::ValidateIncludePreservingProof(
    AcceptedPathKind currentPath, const IncludePatch *patch,
...
::clang::refold::ProofDischargeRecord
RefoldProofLattice::ValidateTUAnchorProof(
