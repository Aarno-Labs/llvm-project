...
//
// Source-line directive parsing and rewriting helpers shared by RefoldEngine's
// include-materialization and expansion-fallback translation units.  These
// definitions were deliberately moved out of the private helper header so that
// line-control rewrite logic has one implementation body instead of many
// inline copies in every engine .cpp that needs the declarations.
//
//===----------------------------------------------------------------------===//
...
bool sourceSuffixMayObservePresumedFileSpelling(
    ...
    const RefoldEngine &engine,
    bool (RefoldEngine::*pathsEqual)(StringRef, StringRef) const,
    StringRef fileText) {
  ...
  std::function<bool(const RefoldModel::MacroInvocation &,
    ...
    if (macro.invFile && !macro.invFile->empty() &&
        (engine.*pathsEqual)(*macro.invFile, file)) {
      if (!macro.invB)
...
rewriteSourceLineDirectiveLogicalLineMacros(
    ...
    ArrayRef<uint64_t> logicalLineSourceOffsets, const RefoldEngine &engine,
    bool (RefoldEngine::*pathsEqual)(StringRef, StringRef) const,
    const LangOptions &lang,
