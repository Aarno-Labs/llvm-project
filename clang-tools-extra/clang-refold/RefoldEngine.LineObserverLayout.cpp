//===--- RefoldEngine.LineObserverLayout.cpp -----------------------*- C++ -*-===//
//
// This file contains RefoldEngine member functions that repair source layout
// when line-state observers such as `__LINE__` remain source-spelled.  These
// routines used to live in the old engine tail-utility fragment; keeping them in a
// real translation unit makes the layout-realization boundary explicit while
// preserving the existing RefoldEngine API and behavior.
//
//===----------------------------------------------------------------------===//
...
lineObserverConstructTokens(
    ...
    const RefoldModel::MacroInvocation &macro, const Engine &engine,
    bool (Engine::*pathsEqual)(StringRef, StringRef) const,
  ...
  for (uint64_t aTok = 0; aTok < static_cast<uint64_t>(aToks.size());
    ...
    if (!(engine.*pathsEqual)(entry.file, ownerFile))
      continue;
...
findLineObserverCollapsedPreGap(
    ...
    uint64_t firstObserverATok, const Engine &engine,
    bool (Engine::*pathsEqual)(StringRef, StringRef) const,
    const DenseMap<uint64_t, RefoldModel::TokMapEntry> &tokmapByPP,
  ...
  for (uint64_t aTok = 1; aTok < firstObserverATok; ++aTok) {
    ...
    if (!(engine.*pathsEqual)(leftEntry.file, ownerFile) ||
        !(engine.*pathsEqual)(rightEntry.file, ownerFile))
      continue;
...
extendLineObserverRightClosure(
    StringRef ownerBytes, StringRef ownerFile, StringRef bSource,
    const Engine &engine, bool (Engine::*pathsEqual)(StringRef, StringRef) const,
    const DenseMap<uint64_t, RefoldModel::TokMapEntry> &tokmapByPP,
    ArrayRef<int64_t> abTokMapA2B, ArrayRef<int64_t> abTokMapB2A,
  ...
  while (true) {
    ...
    if (!(engine.*pathsEqual)(nextEntry.file, ownerFile))
      return closure;
    ...
    for (uint64_t aTok = nextATok + 1;; ++aTok) {
      ...
      if (it == tokmapByPP.end())
        break;
      const RefoldModel::TokMapEntry &entry = it->second;
      if (!(engine.*pathsEqual)(entry.file, ownerFile) || entry.b >= lineEnd)
        break;
...
static std::optional<LineObserverOwnLineMerge> resolveLineObserverOwnLineMerge(
  ...
  if (!(engine.*pathsEqual)(prevEntry.file, ownerFile))
    return std::nullopt;
  ...
  std::optional<LineObserverRightClosure> closure =
      extendLineObserverRightClosure(
          ownerBytes, ownerFile, bSource, engine, pathsEqual, tokmapByPP,
          abTokMapA2B,
...
lineObserverSourceMacroConstructAt(
    ...
    std::optional<uint64_t> ownerIncludeId, const Engine &engine,
    bool (Engine::*pathsEqual)(StringRef, StringRef) const) {
  ...
  for (const RefoldModel::MacroInvocation &macro : macroInvocations) {
    ...
    if (!macro.invFile || !macro.invB || !macro.invE)
      continue;
    if (!(engine.*pathsEqual)(*macro.invFile, ownerFile))
      continue;
