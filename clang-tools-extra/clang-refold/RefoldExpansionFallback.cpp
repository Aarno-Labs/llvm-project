//===--- RefoldExpansionFallback.cpp -----------------------------*- C++ -*-===//
//
// Expansion-fallback and TU include-closure orchestration for RefoldEngine.
// This file replaces the former text-included expansion fallback fragment with
// an ordinary translation unit.  The definitions remain RefoldEngine members;
// only the physical file boundary changed.
//
//===----------------------------------------------------------------------===//
...
#include "IncludeSpellingHelpers.h"
#include "NeutralSourceIslandProof.h"
#include "SourceEnvelopeProof.h"
#include "SourceLineDirectiveHelpers.h"
#include "TokenTextHelpers.h"
#include "ZeroTokenMacroNeutrality.h"
...
#include "RefoldSidebandReplayProof.h"
...
// The legacy partial-expansion proof-bypass path has been removed; the terminal
// resolver below only normalizes a failed structural run onto the declared raw-B
// terminal carrier.  Source-line directive parsing/rewrite helpers live in
// RefoldLineControlRewrite.cpp and are exposed through the private helper header.

// Post-terminal macro expansion is classified through terminal carriers.
//
// A diagnostic-only witness builder used to live here after the synthesis
// proof-bypass path was disabled.  That witness described exactly the same
// owner-closed macro whole-cover surface now represented by the ordinary
// OwnerRealizationProof / MixedOwnerTilingProof lattice before terminal
// fallback is requested.  Keeping a second post-terminal proof search would
// therefore reintroduce a fallback branch, even if it never materialized bytes.
// Terminal fallback now goes directly to the declared TerminalOutOfDomain
// carrier; any in-domain macro whole-cover case must have been accepted by the
...
RefoldEngine::BuildTUIncludeClosureEditForUnresolvedHunk(
  ...
  AuditExpansionFallbackBranchClassification(
      fallbackBranch, "BuildTUIncludeClosureEditForUnresolvedHunk");

  // This source-closure path is still uniquely needed, but it is no longer a
  // legacy unresolved-hunk fallback.  It is the declared TUIncludeClosureEdit
  // proof class: one closed TU source interval may replace top-level include
  // directives plus any adjacent TU-owned tokens consumed by the same hunk,
  // provided all state/layout, B-envelope, and composition obligations below
  // are discharged.  Stay fail-closed unless the closure is exact or can be
  // widened without absorbing any independent token diff.
  if (h.isInsertOnly()) {
  ...
  auto hunkReplacementObservesMacroStateDirective =
      [&](const RefoldModel::MacroDirective &directive) {
        return ReplacementObservesMacroStateDirective(
            directive, SliceBSource(h.bStart, h.bEnd),
  ...
  // The shared helper owns the replacement-list tiling and function-like
  // forwarding mechanics; this TU fallback path supplies only the source
  // surface, path comparison, replacement recovery, and recursive domain.
  ...
  macroInvocationIsSourceNeutralZeroTokenImpl =
    ...
    return refoldMacroInvocationIsSourceNeutralZeroToken(
        model_.GetMacroInvocations(), m, visiting,
        [&](const RefoldModel::MacroInvocation &candidate) {
          return RecoverMacroDefinitionReplacementListInterval(candidate);
        },
        tuBytes, tuPath,
        [&](StringRef lhs, StringRef rhs) { return PathsEqual(lhs, rhs); },
        [](StringRef text) { return isWsOrCompleteCommentTrivia(text); },
        ...
        [&](const RefoldModel::MacroInvocation &child,
          ...
          return macroInvocationIsSourceNeutralZeroTokenImpl(child,
                                                            childVisiting);
        });
  };
  ...
  auto macroDirectiveFullSourceInterval =
      [&](const RefoldModel::MacroDirective &directive) {
        return RecoverMacroStateDirectiveLineInterval(directive, tuPath,
                                                      tuBytes, std::nullopt);
      };
  ...
  if (mixedTUIncludeClosure) {
    ...
    SourceLineDirectiveLogicalLineRewriter sourceLineDirectiveLineRewriter =
      ...
      return rewriteSourceLineDirectiveLogicalLineMacros(
          model_, tuPath, logicalLine, sourceOffsets, *this,
          &RefoldEngine::PathsEqual, lexLang_, std::nullopt,
      ...
      for (size_t idx = 1; idx < mergedPieces.size(); ++idx) {
        ...
        if (!computeSourceLineDirectiveGapResume(
                ...
                !sourceSuffixMayObservePresumedFileSpelling(
                    model_, tuPath, sourceEnd, *this,
                    &RefoldEngine::PathsEqual, tuBytes)))
          continue;
  ...
  for (const RefoldModel::IncludeItem *inc : touched) {
    ...
    // them here would reintroduce the old contiguous-include assumption.
  ...
  auto bEnvelope = ResolveIncludeRealizationBTokenEnvelope(
...
std::string RefoldEngine::ResolvePostStructuralFallback() {
  ...
  AuditExpansionFallbackBranchClassification(terminalBranch,
                                             "ResolvePostStructuralFallback");

  // Reaching this point means every structural proof lattice candidate has
  // failed closed or has been classified out-of-domain.  The old post-terminal
  // macro-expansion proof-bypass audit seam is gone: a macro whole-cover case
  // is in-domain only if OwnerRealizationProof or MixedOwnerTilingProof
  ...
  TerminalFallbackWitness terminalWitness = BuildTerminalFallbackWitness();
  ...
  for (size_t i = 0; i < terminalWitness.proofFailures.size(); ++i) {
    ...
    terminalAuditOk &= AuditTerminalFallbackProofFailure(
  ...
  if (!terminalAuditOk) {
    ...
    terminalWitness = BuildTerminalFallbackWitness();
  }
  ...
  REFOLD_LOG_DEBUG("fallback",
        ...
        terminalFallbackRequests_.size(), terminalWitness.proofFailures.size());
