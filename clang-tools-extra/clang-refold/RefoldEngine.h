...
public:
  ...
  // Public sideband replay proof aliases are kept on RefoldEngine for source
  // compatibility while the canonical proof carriers live in
  // RefoldSidebandReplayProof.h.
  using OwnerLocalSourceEditProof = ::clang::refold::OwnerLocalSourceEditProof;
  using OwnerLocalBReplayProof = ::clang::refold::OwnerLocalBReplayProof;
  using SidebandPragmaEdit = ::clang::refold::SidebandPragmaEdit;

  // Public proof-vocabulary aliases are kept on RefoldEngine for source
  // compatibility while the canonical definitions live in RefoldProofTypes.h.
  using ProofAuditMode = ::clang::refold::ProofAuditMode;
  ...
  using FinalReplaySurface = ::clang::refold::FinalReplaySurface;
...
#undef REFOLD_LEGACY_PATH_KIND_LIST
  ...
  using TheoremFallbackFailureKind =
      ::clang::refold::TheoremFallbackFailureKind;
  ...
  using TerminalFallbackWitness = ::clang::refold::TerminalFallbackWitness;
  using TerminalFallbackRequest = ::clang::refold::TerminalFallbackRequest;
...
#undef REFOLD_MACRO_PATCH_PROOF_KIND_LIST
  ...
  /// stabilization.  A materialized header macro patch may need to consume
  /// earlier #define lines and the callsite line suffix, then re-emit those
  /// definitions after the replacement payload so the B-surface replay sees
  /// the same macro environment that produced it.
  ...
  /// Prove and plan the local macro-state repair for one macro patch emitted
  /// inside a materialized include.
  ///
  /// Return value protocol:
  ///   * std::nullopt: no active header definition is observed by the B text,
  ///     so the original macro patch can be staged unchanged;
  ///   * non-empty StabilizedMaterializedHeaderMacroPatch: stage the widened,
  ///     replay-stable repair interval returned here; and
  ///   * empty {0,0,""}: terminal fallback was requested and materialization
  ///     of this include must stop.
  std::optional<StabilizedMaterializedHeaderMacroPatch>
  StabilizeMaterializedHeaderMacroPatchReplay(
      const MacroPatch &patch, uint64_t patchEnd, llvm::StringRef headerPath,
      uint64_t includeId, llvm::StringRef materializedHeaderBytes,
      llvm::ArrayRef<TextEdit> stagedEdits) const;
