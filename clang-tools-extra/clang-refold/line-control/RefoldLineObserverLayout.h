...
public:
  ...
  /// spelling.  This is a token-boundary safety check for source-preserving macro
  /// rewrite paths; it is static because it depends only on the source slice and
  /// producer macro metadata.
  static bool InvocationSpanMatchesCallsitePrefix(
      llvm::StringRef invSpanText, const RefoldModel::MacroInvocation &m);
  ...
  RefoldLineObserverLayout(
      ...
      : model_(model), bSource_(bSource), aToks_(aToks), bToks_(bToks),
        bTokOff_(bTokOff), abTokMapA2B_(abTokMapA2B),
        abTokMapB2A_(abTokMapB2A), paths_(paths),
        lineControlProof_(lineControlProof), ownerStateProof_(ownerStateProof),
  ...
  bool AppendTURealizationEdits(llvm::StringRef tuPath,
  ...
  LineControlWrappedText WrapIncludeExpansionForMaterialization(
      ...
      std::optional<uint64_t> parentOwnerIncludeId,
      uint64_t parentResumeOffset, size_t childEntryLineNo,
      size_t parentResumeLineNo, llvm::StringRef childBody,
