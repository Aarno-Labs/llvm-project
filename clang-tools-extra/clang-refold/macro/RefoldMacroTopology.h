...
// RefoldModel and then borrowed read-only by callers, so services no longer need
...
///
/// RefoldEngine still owns high-level macro patch orchestration during the
/// staged refactor, but macro identity and traversal are no longer engine
/// internals.  This class preserves the historical lookup policy exactly: if a
/// malformed model contains duplicate macro IDs, the first producer record wins.
...
public:
  ...
  /// True iff \p m is lexically contained in a producer-recorded #define extent.
  ...
  bool SourceRangeInsideConditionalArm(uint64_t condArmId,
                                       llvm::StringRef file,
                                       std::optional<uint64_t> ownerIncludeId,
                                       uint64_t begin, uint64_t end) const;

  /// Return producer-backed A-token ranges for one `__COUNTER__` invocation.
  std::vector<CounterOutputRange>
  ...
  /// inside a macro definition.  Candidate ranking is deterministic and matches
  /// the historical RefoldEngine selector: body-span coverage is preferred over
  /// argument-like coverage, which is preferred over broad cover coverage; ties
...
private:
  ...
  llvm::DenseMap<
      uint64_t, llvm::SmallVector<const RefoldModel::MacroInvocation *, 4>>
      macroChildrenById_;
  ...
  /// Index of widened #define extents keyed by replay-time absolute source path.
