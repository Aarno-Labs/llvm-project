...
bool addFinalMacroCandidate(
  ...
  if (candidate.patch.materialized.hasBTokenRange &&
    ...
    REFOLD_LOG_TRACE(
        "macro/final-candidate",
        "reject inv id={0} name={1} origin={2} reason=hunk-b-range-not-realized "
        "hunkB=[{3},{4}) candidateB=[{5},{6})",
        ctx.invocation.id, ctx.invocation.name,
        finalMacroCandidateOriginName(candidate.origin), ctx.hunk.bStart,
        ctx.hunk.bEnd, candidate.patch.materialized.bTokStart,
        candidate.patch.materialized.bTokEnd);
    return false;
  }
