// RUN: %clang-refold-tester-relaxed pragma_once_operator_midline_conditional_refuses_hoist
// XFAIL: *
...
// dominance proof fails and the refold is refused, reporting
//
//   realized from B needs a top-of-body define
//   no admissible refold
//
...
//
// TRIAGE: incompleteness.  Making this fold needs a third placement that neither
// path has: keeping the define at the site by opening a line for it, so it stays
// inside the arm while the `#ifndef` that skips a repeat inclusion stays at the
// top of the body.  The expected refold below is exactly that shape -- guard
// test at the top, guard define inside the arm -- and it costs one physical line
