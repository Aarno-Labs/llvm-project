...
#undef PREPROCESSOR_OPTION_WITH_MARSHALLING
  ...
  // TENJIN: Capture the current working directory.
...
bool CompilerInvocation::CreateFromArgsImpl(
  ...
  // TENJIN: Capture all of the command line arguments if generating a refold
  // map JSON file.
  auto &PPO = Res.getPreprocessorOpts();
  if (!PPO.RefoldMapFile.empty()) {
    PPO.RefoldPPArgv.clear();
    PPO.RefoldPPArgv.reserve(CommandLineArgs.size());

    for (const char *S : CommandLineArgs) {
      if (!S)
        continue;
      llvm::StringRef SR(S);

      // Optional normalization: drop "-cc1" if it is present in the vector.
      if (SR == "-cc1")
        continue;

      PPO.RefoldPPArgv.emplace_back(SR.str());
    }
  }
